...
\author{Michal Žáček}
\abstractEN{
	NPUs (Neural Processing Units) and AI are getting quite abundant these days,. It seems the market will soon move towards having an NPU accelerator on many devices, for various purposes.

	Boards (such as the Rockchip 3566, 3588 etc.) have usually proprietary user-space libraries to support various input formats of networks, plus some wrapper kernel driver. There is little to no NPU support in the Linux kernel as of now.

	Evaluate the current status of the most important frameworks currently available in this area: OpenVX, Apache TVM, ONNX Runtime, RTLite, Paddle, NNAPI, etc.  What are the advantages/disadvantages of these frameworks? What are these frameworks compatible with? How do these frameworks communicate with hardware?

	Deploy these frameworks and document problems you encounter.

	Check if there are any NPUs with open-source drivers that could be supported in the PikeOS operating system. Alternatively evaluate the complexity of reverse-engineering such a user-space library (this would be interesting to the OSS community as well). Optionally propose a linux+hwvirt-based solution.
}
\abstractCZ{D}
\declaration{D}
\date{2025}
