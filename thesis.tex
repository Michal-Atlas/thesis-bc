...
than the CPU,
about 10x so.
Profiling with \code{perf}
shows 16.28\% taken up by \code{do_lookup_x} from \code{ld-linux.so}
and secondarily 6.61\% being taken up by \code{libOpenVX.so}'s \code{getPatchLocation}.
.\rfc{I'll go investigate what these are now}
